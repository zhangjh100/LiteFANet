...
def generate_segmented_sample_image(scale=1):
    ...
    for i in range(4):
        ...
        for j in range(12):
            img = cv2.imread(r"./images/3D-CBCT-Tooth_segment_result_samples/" + str(i) + "_{:02d}".format(j + 1) + ".jpg")
            img = np.rot90(img, -1)
            if j == 0:
                h, w, _ = img.shape
    ...
    col_names = ["Ground Truth", "UNet3D", "DenseVNet", "AttentionUNet3D", "DenseVoxelNet", "MultiResUNet3D", "UNETR", "SwinUNETR", "TransBTS", "nnFormer", "3DUXNet", "LiteFANet"]
    row_names = ["(a)", "(b)", "(c)", "(d)"]
    ...
    print(image.size)
    image.save(r"./images/3D-CBCT-Tooth_segment_result_samples/3D_CBCT_Tooth_segmentation.jpg")
...
if __name__ == '__main__':
    ...
    # analyse_models(["UNet", "AttU_Net", "CANet", "CENet", "CPFNet", "CKDNet", "SwinUnet", "DATransUNet", "LiteFANet"])
