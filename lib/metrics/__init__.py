    ...
    if opt["dataset_name"] == "3D-CBCT-Tooth" or opt["dataset_name"] == "Kfold-3D-CBCT-Tooth":
        ...
        for metric_name in opt["metric_names"]:
            if metric_name == "DSC":
                ...
                metrics[metric_name] = MMOTU.JI(num_classes=opt["classes"], sigmoid_normalization=opt["sigmoid_normalization"])

            elif metric_name == "ACC":
                metrics[metric_name] = MMOTU.ACC(num_classes=opt["classes"], sigmoid_normalization=opt["sigmoid_normalization"])
            else:
                raise RuntimeError(f"No {metric_name} metric available on {opt['dataset_name']} dataset")
