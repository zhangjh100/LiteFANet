...
class UltraConv_hs(nn.Module):

    def __init__(self, in_planes, out_planes, kernel_size, stride=1, padding=0, dilation=1, groups=1, relu=True, bn=True, bias=False):
        super(UltraConv_hs, self).__init__()
        self.out_channels = out_planes
...
class RFB(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, stride=1, scale=0.1, visual=1):
        ...
        self.branch0 = nn.Sequential(
            UltraConv(in_planes, 2 * inter_planes, kernel_size=1, stride=stride),
            UltraConv(2 * inter_planes, 2 * inter_planes, kernel_size=3, stride=1, padding=visual, dilation=visual, relu=False)
        )
        self.branch1 = nn.Sequential(
            UltraConv(in_planes, inter_planes, kernel_size=1, stride=1),
            UltraConv(inter_planes, 2 * inter_planes, kernel_size=(3, 3), stride=stride, padding=(1, 1)),
            UltraConv(2 * inter_planes, 2 * inter_planes, kernel_size=3, stride=1, padding=visual + 1, dilation=visual + 1, relu=False)
        )
        self.branch2 = nn.Sequential(
            UltraConv(in_planes, inter_planes, kernel_size=1, stride=1),
            UltraConv(inter_planes, (inter_planes // 2) * 3, kernel_size=3, stride=1, padding=1),  # fig 4(a)中5 x 5 conv拆分成两个3 x 3 conv
            UltraConv((inter_planes // 2) * 3, 2 * inter_planes, kernel_size=3, stride=stride, padding=1),
            UltraConv(2 * inter_planes, 2 * inter_planes, kernel_size=3, stride=1, padding=2 * visual + 1, dilation=2 * visual + 1, relu=False)
        )

        self.ConvLinear = UltraConv(6 * inter_planes, out_planes, kernel_size=1, stride=1, relu=False)
        self.shortcut = UltraConv(in_planes, out_planes, kernel_size=1, stride=stride, relu=False)
...
class RFB_hs(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, stride=1, scale=0.1, visual=1):
        ...
        self.branch0 = nn.Sequential(
            UltraConv(in_planes, 2 * inter_planes, kernel_size=1, stride=stride),
            UltraConv_hs(2 * inter_planes, 2 * inter_planes, kernel_size=3, stride=1, padding=visual, dilation=visual, relu=False)
        )
        self.branch1 = nn.Sequential(
            UltraConv(in_planes, inter_planes, kernel_size=1, stride=1),
            UltraConv(inter_planes, 2 * inter_planes, kernel_size=(3, 3), stride=stride, padding=(1, 1)),
            UltraConv_hs(2 * inter_planes, 2 * inter_planes, kernel_size=3, stride=1, padding=visual + 1, dilation=visual + 1, relu=False)
        )
        self.branch2 = nn.Sequential(
            UltraConv(in_planes, inter_planes, kernel_size=1, stride=1),
            UltraConv(inter_planes, (inter_planes // 2) * 3, kernel_size=3, stride=1, padding=1),  # fig 4(a)中5 x 5 conv拆分成两个3 x 3 conv
            UltraConv((inter_planes // 2) * 3, 2 * inter_planes, kernel_size=3, stride=stride, padding=1),
            UltraConv_hs(2 * inter_planes, 2 * inter_planes, kernel_size=3, stride=1, padding=2 * visual + 1, dilation=2 * visual + 1, relu=False)
        )
...
class RFB7a(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, stride=1, scale=0.1, visual=1):
        ...
        self.branch2 = nn.Sequential(
            ...
            UltraConv(inter_planes, (inter_planes // 2) * 3, kernel_size=3, stride=1, padding=1),  # fig 4(a)中5 x 5 conv拆分成两个3 x 3 conv
            UltraConv((inter_planes // 2) * 3, 2 * inter_planes, kernel_size=3, stride=stride, padding=1),
            UltraConv(2 * inter_planes, 2 * inter_planes, kernel_size=3, stride=1, padding=2 * visual + 2, dilation=2 * visual + 2, relu=False)
        )
        self.branch3 = nn.Sequential(
            UltraConv(in_planes, inter_planes, kernel_size=1, stride=1),
            UltraConv(inter_planes, (inter_planes // 2) * 3, kernel_size=3, stride=1, padding=1),  # fig 4(a)中5 x 5 conv拆分成两个3 x 3 conv
            UltraConv((inter_planes // 2) * 3, (inter_planes // 2) * 3, kernel_size=3, stride=1, padding=1),
            UltraConv((inter_planes // 2) * 3, 2 * inter_planes, kernel_size=3, stride=stride, padding=1),
            UltraConv(2 * inter_planes, 2 * inter_planes, kernel_size=3, stride=1, padding=3 * visual + 3, dilation=3 * visual + 3, relu=False)
        )

        self.ConvLinear = UltraConv(8 * inter_planes, out_planes, kernel_size=1, stride=1, relu=False)
        self.shortcut = UltraConv(in_planes, out_planes, kernel_size=1, stride=stride, relu=False)
        self.relu = nn.ReLU(inplace=False)
...
class RFB7a_hs(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, stride=1, scale=0.1, visual=1):
        ...
        self.branch2 = nn.Sequential(
            ...
            UltraConv(inter_planes, (inter_planes // 2) * 3, kernel_size=3, stride=1, padding=1),  # fig 4(a)中5 x 5 conv拆分成两个3 x 3 conv
            UltraConv((inter_planes // 2) * 3, 2 * inter_planes, kernel_size=3, stride=stride, padding=1),
            UltraConv_hs(2 * inter_planes, 2 * inter_planes, kernel_size=3, stride=1, padding=2 * visual + 2, dilation=2 * visual + 2, relu=False)
        )
        self.branch3 = nn.Sequential(
            UltraConv(in_planes, inter_planes, kernel_size=1, stride=1),
            UltraConv(inter_planes, (inter_planes // 2) * 3, kernel_size=3, stride=1, padding=1),  # fig 4(a)中5 x 5 conv拆分成两个3 x 3 conv
            UltraConv((inter_planes // 2) * 3, (inter_planes // 2) * 3, kernel_size=3, stride=1, padding=1),
            UltraConv((inter_planes // 2) * 3, 2 * inter_planes, kernel_size=3, stride=stride, padding=1),
            UltraConv_hs(2 * inter_planes, 2 * inter_planes, kernel_size=3, stride=1, padding=3 * visual + 3, dilation=3 * visual + 3, relu=False)
        )
