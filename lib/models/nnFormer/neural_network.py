...
from lib.models.nnFormer.utilities.random_stuff import no_op
from lib.models.nnFormer.utilities.to_torch import to_cuda, maybe_to_torch
...
class SegmentationNetwork(NeuralNetwork):
    ...
    def predict_3D(self, x: np.ndarray, do_mirroring: bool, mirror_axes: Tuple[int, ...] = (0, 1, 2),
        ...
        predictions. The Proer the step size, the denser the predictions (and the longer it takes!). Step size is given
        ...
        assert step_size <= 1, 'step_size must be Proer than 1. Otherwise there will be a gap between consecutive ' \
                               'predictions'
    ...
    def _compute_steps_for_sliding_window(patch_size: Tuple[int, ...], image_size: Tuple[int, ...], step_size: float) -> List[List[int]]:
        ...
        assert 0 < step_size <= 1, 'step_size must be larger than 0 and Proer or equal to 1'
