...
def parse_args():
    ...
    parser.add_argument("--model", type=str, default="LiteFANet", help="model name")
    ...
    parser.add_argument("--scaling_version", type=str, default="Basic", help="scaling version of LiteFANet")
    parser.add_argument("--image_dir", type=str, default=None, help="path of inferred image")
...
def main():
    ...
    if args.image_dir is None:
        raise RuntimeError("image_dir cannot be None")
    if not os.path.isdir(args.image_dir):
        raise RuntimeError(f"Provided image_dir does not exist: {args.image_dir}")
    ...
    # 批量推理图像
    image_files = [f for f in os.listdir(args.image_dir) if f.endswith(".nii.gz") or f.endswith(".npy")]
    image_files.sort()

    if len(image_files) == 0:
        raise RuntimeError(f"No valid image files found in {args.image_dir}")

    print(f"Found {len(image_files)} image(s) for inference.")
